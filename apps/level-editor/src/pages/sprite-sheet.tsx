          ...
          <For each={zooms}>
            {(z) => (
              <input
                ...
                type="radio"
                name="zoom"
                checked={state.spriteSheetOptions.zoom === z}
                value={z}
                aria-label={`${z}x`}
          ...
          <For each={tileSizes}>
            {(t) => (
              <input
                ...
                type="radio"
                name="tile-size"
                checked={state.spriteSheetOptions.tileSize === t}
                value={t}
                aria-label={`${t}px`}
                on:change={onTileSizeChange}
              />
            )}
          </For>
        </div>
          ...
          <For each={margins}>
            {(m) => (
              <input
                ...
                type="radio"
                name="margin"
                checked={state.spriteSheetOptions.margin === m}
                value={m}
                aria-label={`${m}px`}
        ...
        <div className="inline-block">
          <div className="absolute">
            <div
              ...
              style={{
                padding: `${state.spriteSheetOptions.margin * state.spriteSheetOptions.zoom}px`,
              }}
            >
              <For each={[...new Array(tileCountY())]}>
                {(_, y) => (
                  <div className="flex">
                    <For each={[...new Array(tileCountX())]}>
                      {(_, x) => (
                        <div
          ...
          >
            <img
              src={URL.createObjectURL(state.spriteSheet)}
              style={{
                zoom: state.spriteSheetOptions.zoom,
